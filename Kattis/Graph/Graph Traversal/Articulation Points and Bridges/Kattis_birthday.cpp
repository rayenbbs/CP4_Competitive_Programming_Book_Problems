...
void articulationPointAndBridge(int u,bool &existbridge) {
    ...
    for (auto &v : AL[u]) {
        if (dfs_num[v] == UNVISITED) { // a tree edge
            ...
            articulationPointAndBridge(v,existbridge);
            if (dfs_low[v] >= dfs_num[u]) {
                articulation_vertex[u] = 1;
            }
            if (dfs_low[v]>dfs_num[u])
                existbridge=true;// store this info first
    ...
    while (N!=0 || c!=0)
    {
        ...
        for (int u = 0; u < N; ++u)
        {
            if (dfs_num[u] == UNVISITED)
            {
                dfsRoot = u;
                rootChildren = 0;
                articulationPointAndBridge(u,existbridge);
